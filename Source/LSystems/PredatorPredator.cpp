...
// Sets default values
APredatorPredator::APredatorPredator()
{
	...
	USkeletalMesh* usm = LoadObject<USkeletalMesh>(this, TEXT("SkeletalMesh'/Game/AnimalVarietyPack/Wolf/Meshes/SK_Wolf.SK_Wolf'"));
	...
	theTexture = APredatorPredator::LoadTexture(64, 0.35, 3.0, 4.0, 6.0, 6.0);
	...
	walking_anim = LoadObject<UAnimSequence>(this, TEXT("AnimSequence'/Game/AnimalVarietyPack/Wolf/Animations/ANIM_Wolf_Walk.ANIM_Wolf_Walk'"));
	running_anim = LoadObject<UAnimSequence>(this, TEXT("AnimSequence'/Game/AnimalVarietyPack/Wolf/Animations/ANIM_Wolf_Run.ANIM_Wolf_Run'"));
	eating_anim = LoadObject<UAnimSequence>(this, TEXT("AnimSequence'/Game/AnimalVarietyPack/Wolf/Animations/ANIM_Wolf_Bite.ANIM_Wolf_Bite'"));
	dying_anim = LoadObject<UAnimSequence>(this, TEXT("AnimSequence'/Game/AnimalVarietyPack/Wolf/Animations/ANIM_Wolf_Death.ANIM_Wolf_Death'"));
	walking_anim->RateScale = 0.68;

}

// Called when the game starts or when spawned
void APredatorPredator::BeginPlay()
{
...
bool APredatorPredator::is_inside_ellipse(float h, float k, float x, float y, float a, float b) {
	return (((x - h) * (x - h)) / (a * a) + (((y - k) * (y - k)) / (b * b))) <= 1;
}

int APredatorPredator::inner(int i, int j, uint8* Pixels, int size, float inner_range_x, float inner_range_y) {
	int count = 0;
	for (int k = i - inner_range_x; k < i + inner_range_x; k++) {
		for (int l = j - inner_range_x; l < j + inner_range_y; l++) {
			int32 curPixelIndex = ((k * size) + l);
			if (Pixels[4 * curPixelIndex + 0] == 255 && APredatorPredator::is_inside_ellipse(i, j, k, l, inner_range_x, inner_range_y)) {
				count++;
			}
		}
	}
...
int APredatorPredator::outter(int i, int j, uint8* Pixels, int size, float inner_range_x, float inner_range_y, float outer_range_x, float outer_range_y) {
	int count = 0;
	for (int k = i - outer_range_x; k < i + outer_range_x; k++) {
		for (int l = j - outer_range_y; l < j + outer_range_y; l++) {
			int32 curPixelIndex = ((k * size) + l);
			if (Pixels[4 * curPixelIndex + 0] == 255 && APredatorPredator::is_inside_ellipse(i, j, k, l, outer_range_x, outer_range_y)) {
				if (!APredatorPredator::is_inside_ellipse(i, j, k, l, inner_range_x, inner_range_y)) {
					count++;
				}
			}
		}
...
UTexture2D* APredatorPredator::LoadTexture(int size, float ratio, float inner_range_x, float inner_range_y, float outer_range_x, float outer_range_y)
{
	...
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < size; j++) {
			for (int k = 0; k < size; k++) {
				...
				int activator = APredatorPredator::inner(j, k, Pixels, size, inner_range_x, inner_range_y);
				int inhibitor = APredatorPredator::outter(j, k, Pixels, size, inner_range_x, inner_range_y, outer_range_x, outer_range_y);
...
// Called every frame
void APredatorPredator::Tick(float DeltaTime)
{
	...
	if (eating) {
		...
		FVector preyPos = { 0,0,0 };
		APredatorPawn* prey = nullptr;
		...
		for (FConstPawnIterator Itr = GetWorld()->GetPawnIterator(); Itr; ++Itr)
		{
			...
			{
				if (!IsValid(prey)) {
					preyPos = Itr->Get()->GetActorLocation();
					prey = (APredatorPawn*)Itr->Get();
				}
				if (FVector::Dist(Itr->Get()->GetActorLocation(), GetActorLocation()) < FVector::Dist(preyPos, GetActorLocation())) {
					preyPos = Itr->Get()->GetActorLocation();
					prey = (APredatorPawn*)Itr->Get();
				}
			}
		...
		if (energy < 200 && IsValid(prey) && prey->IsAlive()) {
			NewLocation += (DeltaTime * (preyPos - GetActorLocation()).GetSafeNormal()); //Going to prey
		}
		if (energy < 200 && IsValid(prey) && FVector::Dist(prey->GetActorLocation(), GetActorLocation()) < 150 && prey->IsAlive()) {
			prey->Attacked(20);
			energy += 20;
...
void APredatorPredator::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
